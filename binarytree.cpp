...
{
  printLeftToRight(root); //root or subroot? - start from the top 
  //public private default? - in .h
  // why a node pointer (for recursion)

    // Do not remove this line - rpused for correct print output
  cout << endl;
}
...
void BinaryTree<T>::mirror(Node* subroot)
{
    ...
    std::swap(subroot->left, subroot->right); //only playing with pointers
    mirror(subroot->left);
...
template <typename T>
void BinaryTree<T>::printPaths(const Node* node, string path) const
{

  /* if (node == NULL) return;
  path += to_string(node->elem) + " ";
  ...
  printPaths(node->right, path); */
   ...
   if(node == NULL) {
    cout << path << endl;
      ...
      printPaths(node->left, path + to_string(node->elem) + " ");
    } else if (node->left == NULL) {
      printPaths(node->right, path + to_string(node->elem) + " ");
    } else {
      printPaths(node->left, path + to_string(node->elem) + " ");
      printPaths(node->right, path + to_string(node->elem) + " ");
    }  
...
template <typename T>
int BinaryTree<T>::sumDistances(Node * node, int sum) const
{
  if (node == NULL) return 0;
  return sum + sumDistances(node->left, sum + 1) + sumDistances(node->right, sum + 1);
}
